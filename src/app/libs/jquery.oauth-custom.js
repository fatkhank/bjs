...
jqOAuth.prototype.hasAccessToken = function hasAccessToken() {
    return this.data.accessToken != null;//kalau null dianggap punya, walaupun defined
};
...
jqOAuth.prototype._fireBuffer = function _fireBuffer() {
    ...
    $.when.apply($, promises)
        ...
        .fail(function (error) {
            ...
            //CUSTOM: its commented, so error when firebuffer not automatically logout
            //self.logout();
            return error;
        });
};
...
jqOAuth.prototype._getStoredData = function _getStoredData() {
    $.extend(this.data, this.options.store.get(this.options.tokenName));
};
...
jqOAuth.prototype._hasStoredData = function _hasStoredData() {
    return this.options.store.get(this.options.tokenName) !== undefined;
};
                ...
                if (!self.refreshing) {
                    ...
                    self._fireEvent("tokenExpiration")
                        .then(function () {
                            ...
                            //CUSTOM: commented, error handling pass to oauth.ts
                            // self.logout();
                            return error;
                        });
                }
...
jqOAuth.prototype._updateStorage = function _updateStorage() {
    this.options.store.set(this.options.tokenName, this.data);
};
